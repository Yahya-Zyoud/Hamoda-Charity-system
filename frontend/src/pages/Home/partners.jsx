...
import { Handshake, Star, Briefcase, Landmark, Building2, Building, Crown, AlertTriangle, Hospital, HeartPulse } from "lucide-react";
...
function PartnerCard({ partner }) {
  ...
  return (
    <div className="flex-shrink-0 w-64 bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-lg hover:scale-105 transition-all duration-300 cursor-pointer group">
      <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform overflow-hidden`}>
        {partner.logo ? (
      ...
      <p className="text-blue-950 font-bold text-sm leading-tight text-center">{partner.name}</p>
      <div className={`mt-2 h-1 w-8 rounded-full bg-gradient-to-r ${color} group-hover:w-full transition-all duration-500`} />
    </div>
  );
}
    ...
    <section className="py-20 bg-gray-50 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6 mb-12 text-center">
        ...
        <h2 className="text-4xl font-black text-blue-900 mb-3">
          نعمل مع أفضل{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-800 to-blue-600">الشركاء</span>
        </h2>
        <p className="text-gray-500 text-xl max-w-xl mx-auto">شركاء موثوقون يدعمون مسيرتنا لخدمة المجتمع</p>
      </div>

      {error && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          <div className="text-red-500 bg-red-100 p-3 rounded-full mb-2">
            <AlertTriangle className="w-10 h-10" />
          </div>
          <p className="text-red-600 font-semibold text-lg">حدث خطأ أثناء تحميل البيانات</p>
          <button onClick={loadPartners} className="px-6 py-3 bg-blue-600 text-white rounded-xl text-base font-semibold shadow-md transition duration-200 hover:bg-blue-700 hover:scale-[1.02]">
            إعادة المحاولة
          </button>
        </div>
      )}
      ...
      {loading && !error && (
        <div className="flex gap-6 px-6 overflow-hidden">
          {[1, 2, 3, 4, 5].map((n) => <PartnerSkeleton key={n} />)}
        </div>
      )}
      ...
      {!loading && !error && partners.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 gap-3 text-gray-400">
          ...
          <p className="text-gray-500 font-semibold">لا يوجد شركاء مضافون حالياً</p>
        </div>
      )}
    </section>
  );
