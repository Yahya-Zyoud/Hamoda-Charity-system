      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-800 mb-8 leading-tight tracking-tight">
            مشاريع تصنع <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-blue-600">الأثر المستدام</span>
          </h2>
        ...
        {error && (
          <div className="flex flex-col items-center justify-center p-12 bg-red-50/80 backdrop-blur-sm rounded-3xl border border-red-100 text-center gap-5 max-w-2xl mx-auto">
            ...
            <button onClick={fetchProjects} className="mt-2 px-8 py-3.5 bg-red-600 text-white rounded-xl text-lg font-bold shadow-md hover:bg-red-700 transition">
              المحاولة مرة أخرى
            </button>
          </div>
        )}
              ...
              {projects.map((p, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`group p-6 md:p-8 rounded-[2rem] transition-all duration-500 cursor-pointer border ${isActive ? 'bg-white shadow-[0_20px_50px_rgb(0,0,0,0.06)] border-white scale-[1.02] z-10' : 'bg-transparent border-transparent hover:bg-white/40'}`}
                  >
                    <div className="flex gap-5 md:gap-8 items-start">

                      <div className={`w-14 h-14 md:w-16 md:h-16 rounded-[1.5rem] flex items-center justify-center shrink-0 transition-all duration-500 ${isActive ? `bg-gradient-to-br ${design.color} text-white shadow-lg ${design.shadow} -translate-y-1` : 'bg-slate-100 text-slate-400 group-hover:bg-slate-200'}`}>
                      ...
                      <div className="flex-1">
                        ...
                        <div className={`flex items-center justify-between overflow-hidden transition-all duration-500 ${isActive ? 'max-h-16 opacity-100 translate-y-0' : 'max-h-0 opacity-0 translate-y-4'}`}>
                          <button className={`flex items-center gap-2 font-bold text-sm bg-clip-text text-transparent bg-gradient-to-l ${design.color}`}>
                            تفاصيل إضافية <ArrowLeft className={`w-4 h-4 text-slate-800 rtl:-scale-x-100`} />
                          </button>
